(() => {
  ...
  function withBust(url) {
    ...
    u.searchParams.set('_feedbackTrend', `${Date.now()}-${Math.random().toString(36).slice(2, 7)}`);
    return u.toString();
  }
  ...
  function validRegistrations(events) {
    const byKey = new Map();
  ...
  function buildTrend(events) {
    const regs = validRegistrations(events);
    const starts = events
  ...
  function lineChart(runs) {
    ...
    return `<div class="feedback-chart-scroll"><svg class="feedback-chart" viewBox="0 0 ${width} ${height}" style="width:${width}px" role="img" aria-label="Historical percentage of students reporting at least 80 percent understanding"><g>${grid}<polyline points="${points}" class="feedback-line"/>${dots}</g></svg></div>`;
  }
  ...
  function cardHtml(runs) {
    ...
    return `<section id="feedbackHistoricalTrend" class="feedback-trend-card">
  ...
  function injectStyles() {
    if (document.getElementById('bda-feedback-trend-styles')) return;
    const s = document.createElement('style');
    s.id = 'bda-feedback-trend-styles';
    s.textContent = `
      .feedback-trend-card{margin:18px 0 0;border:1px solid #dedede;border-radius:18px;background:#fff;padding:20px;box-shadow:0 2px 10px rgba(0,0,0,.025)}
  ...
  function place(html) {
    const page = document.querySelector('#app .page.present');
    if (!page || !isFeedbackPage()) return;
    const existing = document.getElementById('feedbackHistoricalTrend');
    if (existing && existing.outerHTML === html) return;
    if (existing) existing.remove();
    const grid = page.querySelector('.presenter-grid');
    if (!grid) return;
    grid.insertAdjacentHTML('afterend', html);
  }

  async function refresh() {
    if (busy || !isFeedbackPage()) return;
    busy = true;
    try {
      injectStyles();
      const events = await loadEvents();
      const runs = buildTrend(events);
      const html = cardHtml(runs);
      lastHtml = html;
      place(html);
    } catch (e) {
      console.warn('BDA feedback trend:', e);
      if (lastHtml) place(lastHtml);
    } finally {
      busy = false;
    }
  }
  ...
  setInterval(() => {
    ...
    if (lastHtml && !document.getElementById('feedbackHistoricalTrend')) place(lastHtml);
    refresh();
  }, REFRESH_MS);
